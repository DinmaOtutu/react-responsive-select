        ...
        >
          {customLabelText && (
            <div className="rrs__label">
              <span aria-label={this.getAriaLabel()} className="rrs__label__text" id={`rrs-${name}-label`}>
                {customLabelText}
              </span>
              {caretIcon && caretIcon}
            </div>
          ...
          {name && (
            <input type="hidden" name={name} value={[multiSelectSelectedOptions.options.map(v => v.value)].join(',')} />
          )}
        </div>
