      ...
      const newState = {
        ...
        nextPotentialSelectionIndex: (() => {
          if (state.multiselect) {
            return state.multiSelectSelectedIndexes.length ? state.multiSelectSelectedIndexes[0] : 0;
          }
    ...
    case actionTypes.SET_MULTISELECT_OPTIONS: {
      if (!state.noSelectionLabel) {
        ...
        const shouldDeselectAllAndSelectFirstOption =
          state.multiSelectSelectedIndexes.length > 0 && !isFirstOptionInListSelected && action.optionIndex === 0;
